...
$(document).ready(function () {
    ...
    $('#puzzlearea div').each(UtilityFunctions.positioningCss);

    $('.puzzlepiece').click(UtilityFunctions.puzzlePieceClick);


    $('#shufflebutton').click(UtilityFunctions.shufflePieces);

    
    $('.puzzlepiece').hover(UtilityFunctions.puzzlePieceHover, function (){ $(this).removeClass('movablepiece');});

});

let UtilityFunctions = {
    //function for setting the position and css for the filled squares
    positioningCss:function(){
        var filledSquares = {};
        var x = ((count % 4) * 100);
...
let HelperFunctions = {
    ...
    clickableleft : function (changedleft, changedtop)
    {
    ...
    //helper function for finding the neighbouring elements on the top
    clickableTop : function (changedleft, changedtop)
    {
